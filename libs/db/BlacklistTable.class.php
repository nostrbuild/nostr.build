...
class BlacklistTable extends DatabaseTable
{
  public function __construct(mysqli $db)
  {
    parent::__construct($db, 'blacklist');
    $this->requiredValidationRules = [
      'filename' => v::notEmpty()->stringType()->length(null, 255),
    ];
    $this->optionalValidationRules = [
      'id' => v::intVal(),
      'npub' => v::optional(v::stringType()->length(1, 255)),
    ];
  }
  ...
  {
    $sql = "SELECT id, npub, timestamp, reason FROM " . $this->tableName . " WHERE id > ? ORDER BY id ASC LIMIT ?";
    try {
      $bindId = $id;
      $bindLimit = $limit;
